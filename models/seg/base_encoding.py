...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
        ...
        if backbone == 'resnet50':
            ...
            # resnet_model = resnet.resnet50(pretrained=True, dilated=dilated,
            #                                norm_layer=norm_layer, root=root)
            # features = list(resnet_model)
            # self.pretrained = nn.ModuleList(features)
            ...
            mobilenet_v2_model = mobilenet.mobilenet_v2(pretrained=True, root=root,
                                                        norm_layer=norm_layer)
            features = list(mobilenet_v2_model.features)
            self.pretrained = nn.ModuleList(features)
            # self.pretrained = mobilenet_v2_model.features
        else:
    ...
    def base_forward(self, x):
        if self.backbone.startswith('wideresnet'):
            ...
            for i, layer in enumerate(self.pretrained):
                if i == 3:
                    layer_tmp = layer._modules
                    fourth_low_level_features = layer_tmp['conv'][:1](x)
                    x = layer_tmp['conv'](x)
                if i == 6:
                    layer_tmp = layer._modules
                    eighth_low_level_features = layer_tmp['conv'][:1](x)
                    x = layer_tmp['conv'](x)
                if i == 13:
                    layer_tmp = layer._modules
                    sixteen_features = layer_tmp['conv'][:1](x)
                    x = layer_tmp['conv'](x)
                else:
                    if i < 18:
                        x = layer(x)
            return x, fourth_low_level_features, eighth_low_level_features, sixteen_features
        elif self.backbone.startswith('resnet'):
            x = self.pretrained.conv1(x)  # may occur error
            x = self.pretrained.bn1(x)
            x = self.pretrained.relu(x)
            x = self.pretrained.maxpool(x)
            c1 = self.pretrained.layer1(x)
            fourth_low_level_features = c1.clone()
            c2 = self.pretrained.layer2(c1)
            eighth_low_level_features = c2.clone()
            c3 = self.pretrained.layer3(c2)
            sixteen_features = c3.clone()
            c4 = self.pretrained.layer4(c3)
            return c4, fourth_low_level_features, eighth_low_level_features, sixteen_features
        else:
            x = self.pretrained.conv1(x)  # may occur error
            x = self.pretrained.bn1(x)
            x = self.pretrained.relu(x)
